...
DWORD dmm::create_mailbox(mbufer **ppMbufer, WORD wMsgAddr, const CHAR *pcTaskName)
{
    ...
    {
        case ADDR_MstTask:
            stLclAddr.sin_addr.s_addr = inet_addr(IP_0);
            stLclAddr.sin_port = htons(PORT_0);
            break;

        case ADDR_SlvTask1:
            stLclAddr.sin_addr.s_addr = inet_addr(IP_11);
            stLclAddr.sin_port = htons(PORT_11);
            break;
...
DWORD mbufer::send_message(WORD wDstAddr, void *pData, WORD wDataLen)
{
    ...
    {
        case ADDR_MstTask:
            stDstAddr.sin_addr.s_addr = inet_addr(IP_0);
            stDstAddr.sin_port = htons(PORT_0);
            break;

        case ADDR_SlvTask1:
            stDstAddr.sin_addr.s_addr = inet_addr(IP_11);
            stDstAddr.sin_port = htons(PORT_11);
            break;
